...
Defs::Defs(int argc, char *argv[])  {
    ...
    mRoot = environ.value("IFAOE_BIRD_CENSUS",mHome+"/"+ROOT_IFAOE_BIRD_CENSUS);
    mEnv  = environ.value(ENV_IFAOE_BIRD_CENSUS,TK_QSTR_NONE);
    if (mEnv!=TK_QSTR_NONE) { mRoot=mEnv;  }

    fRoot = QFileInfo( mRoot );
    if ( !fRoot.isDir() && !fRoot.isReadable() ) {
        qFatal("%s: Unbekanntes Arbeitsverzeichnis %s ",
               KEY_IFAOE_BIRD_CENSUS,
    ...
    if ( cfgFile.compare(TK_QSTR_NONE) == 0) {
       tmp = mRoot+"/Konfiguration/Applikation.cfg";
    } else {
       tmp = cfgFile;
    }
    fConf = QFileInfo( tmp );
    if ( !fConf.isFile() && !fConf.isReadable() ) {
        qFatal("%s: Unbekanntes Startdatei %s ",
               KEY_IFAOE_BIRD_CENSUS,
               tmp.toStdString().c_str());
    }
}

QString Defs::getConfig() const { return fConf.absoluteFilePath(); }
QString Defs::getEtcPath()    const { return fEtc.absoluteFilePath(); }
QString Defs::getSymbolPath() const { return fSymbol.absoluteFilePath(); }
QString Defs::getRoot() const { return mRoot; }
QString Defs::getHome() const { return mHome; }
QString Defs::getUser() const { return mUser; }
